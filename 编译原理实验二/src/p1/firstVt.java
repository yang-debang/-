...
public class firstVt {
   ...
   public void putinstack(char a,char b){//将非终a，和终结b合成字符串，再放入栈中
       ...
       if(a=='S'){//每一个进栈的元素都确定终结符是非终结符的first集，则在整型二维数组中定义每个元素坐标表达的信息
          if(b=='#'){biogroup[0][0]=1;}
          if(b=='+'){biogroup[0][1]=1;}
          if(b=='*'){biogroup[0][2]=1;}
          if(b=='('){biogroup[0][3]=1;}
   ...
   public void analyse(){//栈的初始化

       for(int i1=0;i1<4;i1++){
           if(production[i1][2]!=null){
               if(production[i1][2].charAt(0)<65||production[i1][2].charAt(0)>=97) {
                   //因为第二个右部产生式是单字符，右部产生式首字符是终结符，则压入栈中
                   putinstack(production[i1][0].charAt(0),production[i1][2].charAt(0));
               }
           }
           for(int i=0;i<2;i++) {

                if (production[i1][1].charAt(i)<65) {
                   //右部产生式前二个若是终结符，则压入栈
                   putinstack(production[i1][0].charAt(0),production[i1][1].charAt(i));
    ...
    */
    public void recursive (){
       if(stackLength==0){//设定递归结束条件
           System.out.println("栈空了" );
           return;
       }
       String stirngstack=outstack().trim();//执行一次出栈，然后保存好第一、第二个字符
       char a=stirngstack.charAt(0);
       char b=stirngstack.charAt(1);

        for(int i1=0;i1<4;i1++) {
             if(production[i1][2]==null){//因为第二个产生式有空的情况，所以单独对这种情况处理
                 if(a==production[i1][0].charAt(0)){}//出栈的首字符若和左部相同，则跳过，避免出现死循环
                 else if(a==production[i1][1].charAt(0)){
                     //若右部第一个产生式首字符和出栈的非终相同，则将该产生式的左部和终结符一起压入栈
                     putinstack(production[i1][0].charAt(0),b);
                 }
             }
            else if (a==production[i1][1].charAt(0)||a==production[i1][2].charAt(0)){
                if(a==production[i1][0].charAt(0)){}//出栈的首字符若和左部相同，则跳过，避免出现死循环
                else {
                    //若右部第一个产生式首字符和出栈的非终相同，则将该产生式的左部和终结符一起压入栈
                    putinstack(production[i1][0].charAt(0), b);
